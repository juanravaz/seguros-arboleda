                        ...
                        <div class="card-datatable table-responsive pt-0">
                            ...
                            <table class="user-list-table table tabla" aria-describedby="table" id="table" >
                                ...
                                @foreach($lifeInsurances as $lifeinsurance)
                                    <tr>
                                        <td>{{$lifeinsurance->insurance->name}}</td>
                                        <td>{{$lifeinsurance->insurance->price}} &euro;</td>
                                        <td>{{$lifeinsurance->DNI}}</td>
                                        <td>{{$lifeinsurance->Name}}</td>
                                        <td>{{$lifeinsurance->Birth}}</td>

                                        <td style="text-align: center;">
                                            <div class="container">
                                                <div class="row">
                                                    <div class="col">
                                                        <a href="{{ url('/home/insurances/edit/'.$lifeinsurance->insurance->id)}}" class="btn btn-xs btn-warning">Editar</a>
                                                    </div>
                                                    <div class="col">
                                                        <form method="POST" action="{{ route('admin.lifeinsurance.destroy') }}">
                                                            @csrf
                                                            @method('DELETE')
                                                            <input type="hidden" name="home" value="{{ $lifeinsurance->insurance->id }}">
