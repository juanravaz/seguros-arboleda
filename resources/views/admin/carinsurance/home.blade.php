                    ...
                    <div class="card">
                        <div class="card-datatable table-responsive pt-0">
                            ...
                            <table class="user-list-table table tabla" aria-describedby="table" id="table" >
                                ...
                                @foreach($carInsurances as $carinsurance)
                                    <tr>
                                        <td>{{$carinsurance->insurance->name}}</td>
                                        <td>{{$carinsurance->insurance->price}} &euro;</td>
                                        <td>{{$carinsurance->plate}}</td>
                                        <td>{{$carinsurance->fuel}}</td>
                                        <td>{{$carinsurance->car}}</td>
                                        <td>{{$carinsurance->year}}</td>

                                        <td style="text-align: center;">
                                            <div class="container">
                                                <div class="row">
                                                    <div class="col">
                                                        <a href="{{ url('/car/insurances/edit/'.$carinsurance->insurance->id)}}" class="btn btn-xs btn-warning">Editar</a>
                                                    </div>
                                                    <div class="col">
                                                        <form method="POST" action="{{ route('admin.carinsurance.destroy') }}">
                                                            @csrf
                                                            @method('DELETE')
                                                            <input type="hidden" name="home" value="{{ $carinsurance->insurance->id }}">
